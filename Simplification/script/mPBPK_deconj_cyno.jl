# author: Yuezhe LI
# date: Apr 23, 2024
# purpose of the code: to simplify Jones model with ADC deconjugation in cyno
# prior Jones model: see https://ghe.metrumrg.com/yuezhel/cyno_pbpk/blob/main/jonesODEs3Monkey.jl
...
function mPBPK_deconj_cyno!(du,u,p,t) 
    ...
    @unpack infusion, tumor_cell_radius, tdouble, tau, PS_Score, PS_kd, KD6_WT, DAR, 
            epsilon, P_ADC, D_ADC, Rcap, Rkrogh, thalf_sR, thalf_sR_adc, init_sR, k_kill_max, ic50_pl, k_PL, 
            Rcopies, k_deg, k_rec, k_endo, Kon, Kd, k_out, k_in, k_lys, k_dec = p
    ...
    # Vascular Volumes [L]
    # based on monkey BW = 6.2 kg
    V_V[Lung] = 1.96/1000
    V_V[Liver] = 15.9/1000
    V_V[Skin] = 25.2/1000
    V_V[Kidney] = 1.5/1000
    V_V[Intestine] = (1.57+2.24+0.685)/1000 # combined SI, LI, Pancreas
    V_V[Spleen] = 0.72/1000
    V_V[Other] = (1.09+72+1.7+20.9+2.07+0.115+5.56)/1000     # combine Heart, Muscle, Adipose, Bone, Brain, Thymus, and Other
    ...
    V_IntS[Intestine] = (17.2+24.4+2.17)/1000   # combined SI, LI, Pancreas
    V_IntS[Spleen] = 1.19/1000
    V_IntS[Other] = (4.05+426+26.3+177+16.9+0.355+22.7)/1000      # combine Heart, Muscle, Adipose, Bone, Brain, Thymus, and Other
    ...
    V_Organ[Intestine] = (98.7+140+12.5)/1000   # combined SI, LI, Pancreas
    V_Organ[Spleen] = 5.95/1000
    V_Organ[Other] = (28.3+3273+154+952+94+2.09+132)/1000    # combine Heart, Muscle, Adipose, Bone, Brain, Thymus, and Other

    # scaled linearly based on the BW using ref. BW = 6.2 kg
    V_Plasma = 0.187; # [L]
    V_LN     = 25.1/1000; # [L]

    # Blood Flows [L/h]
    # scaled with an allometric exponent of 0.75 based on the BW using ref. BW = 6.2 kg
    # PLQ[Lung] = 22433/1000 
    PLQ[Liver] = 1251/1000
    PLQ[Skin] = 2492/1000
    PLQ[Kidney] = 3237/1000
    PLQ[Intestine] = (3432+3571+399)/1000      # combined SI, LI, Pancreas
    PLQ[Spleen] = 185/1000
    PLQ[Other] = (696+3944+139+248+1508+125+1206)/1000       # combine Heart, Muscle, Adipose, Bone, Brain, Thymus, and Other
    # rebalance PLQ
    ...
    Scale_Factor = 60# scaling factor for human; if mouse, this number should be 1; monkey is optimized in https://ghe.metrumrg.com/yuezhel/cyno_pbpk
    ...
    #  All organs except Lung and Liver at vascular cmpt (below), exogenous and endogenous IgG, free only
        ...
        )/V_V[3:N_Organs] - k_dec.*C_EXG[3:N_Organs,V]); 
    ...
    Kkill_eff = k_kill_max*C_P_c/(ic50_pl + C_P_c)
    ...
    Kkill_eff_neg = k_kill_max*C_P_neg_c/(ic50_pl + C_P_neg_c)
    ...
    du.C_EXG_Plasma = ((w_other_1_EXG_Plasma - flux_ADC_plasma2tumor)/V_Plasma - k_dec*C_EXG_Plasma - Kon*C_sR_plasma*C_EXG_Plasma + Koff_sR_EXG*C_sR_EXG) + infusion; 
    ...
    du.DEGprotein.deg_plasma = kdeg_sR_adc*C_sR_EXG + k_dec*C_EXG_Plasma*V_Plasma + sum(C_EXG[1:N_Organs,V].*V_V[1:N_Organs].*k_dec)
