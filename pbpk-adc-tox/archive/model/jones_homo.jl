...
# in addition, soluble HER in plasma dynamics (also from Scheuher et al., 2024); https://pubmed.ncbi.nlm.nih.gov/37787918/
# unit: concentratio [uM]; volume [L]; time [hr];


function jonesODEs_homo_tumor!(du,u,p,t) 
    @unpack C_EXG_Plasma, C_sR_plasma, C_sR_EXG, C_EDG_Plasma, C_EDG_LN, C_EDG, C_EXG_LN, C_EXG, C_FcRn_E6a, C_FcRn_E7, C_FcRn_E7b, C_FcRn_ISM, C_FcRn_VM, 
            tumor_ADC, DEGprotein, DegPL, end_cyto_payload, ints_payload, plasma_payload = u
        
    @unpack infusion, PS_Score, PS_kd, KD6_WT, Prob_deg, kint_PS, DAR, k_deconj, 
            epsilon, P_ADC, D_ADC, Rcap, Rkrogh, thalf_sR, thalf_sR_adc, init_sR, Kon, Kd, k_out, frac_lys, k_PL, k_PL_ex, CL_PL_plasma, 
            TumorVolume_L, lesion_num  = p
    ...
    PLQ[Other] = 910/1000              # modified after adding the BM compartment, 9190 - 8280 = 910
    PLQ[Marrow] = 8280/1000             # From Baxter_et_al_1995             
    # rebalance PLQ
    ...
    LF[Marrow]   = PLQ[Marrow]*LF_PLQ_frac
    ...
    + (1-sigma_IS[Marrow])*LF[Marrow]*C_EDG[Marrow, IntS]
    ...
    +(PLQ[Marrow]-LF[Marrow])*C_EXG[Marrow, V]
    ...
    + (1-sigma_IS[Marrow])*LF[Marrow]*C_EXG[Marrow, IntS]
    ...
    # tumor dynamics
    V_IntS_Tumor = 0.55 * TumorVolume_L; # Lindauer et al., 2017; https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5270293/


    # vascular & surface-based exchange of  tumor
    Avg_lesion_volume = TumorVolume_L / lesion_num
    Rtumor = ( Avg_lesion_volume * 1e3/(4/3*pi) ) ^ (1/3) # [cm]
    surface_exchange = 6 * D_ADC/ (Rtumor^2) ; # [h-1]
    vascular_exchange = 2 * P_ADC * Rcap / (Rkrogh^2) ; # [h-1]

    flux_ADC_plasma2tumor = (vascular_exchange + surface_exchange) * (C_EXG_Plasma * epsilon - tumor_ADC) * V_IntS_Tumor

    # update ADC conc in plasma
    du.C_EXG_Plasma = ((w_other_1_EXG_Plasma - flux_ADC_plasma2tumor)/V_Plasma - k_deconj*C_EXG_Plasma - Kon*C_sR_plasma*C_EXG_Plasma + Koff_sR_EXG*C_sR_EXG) + infusion; 
    ...
    # payload release from ADC 
    @views @. du.DegPL.deg_membrane[1:N_Organs] = (CL_up[1:N_Organs]*Prob_deg*C_EXG[1:N_Organs,E6a])*DAR
    @views @. du.DegPL.deg_FcRn[1:N_Organs] = (kdeg_FcRn_Ab*C_EXG[1:N_Organs,bound_VM]*V_VM[1:N_Organs] + kdeg_FcRn_Ab*C_EXG[1:N_Organs,bound2_VM]*V_VM[1:N_Organs] + kdeg_FcRn_Ab*C_EXG[1:N_Organs,bound_ISM]*V_ISM[1:N_Organs] + kdeg_FcRn_Ab*C_EXG[1:N_Organs,bound2_ISM]*V_ISM[1:N_Organs])*DAR

    # payload concentration in tissue endothetial cells' cytosol
    V_cytosol = V_Organ - V_V - V_IntS - V_endosomal
    @views @. du.end_cyto_payload = ((CL_up[1:N_Organs]*Prob_deg*C_EXG[1:N_Organs,E6a]+ 
    ...
    )/V_cytosol[1:N_Organs]*DAR * frac_lys  # release of payload from ADC
        - k_out*(end_cyto_payload[1:N_Organs] .- ints_payload[1:N_Organs])  # payload from cytosol to tissue interstitium 
        - k_out*(end_cyto_payload[1:N_Organs] .- plasma_payload)  # payload from endothelial cell cytosol to plasma 
    )

    # payload concentration in plasma 
    du.plasma_payload = sum(k_out*(end_cyto_payload[1:N_Organs] .- plasma_payload) .* V_cytosol[1:N_Organs])/V_Plasma + k_deconj*C_EXG_Plasma*DAR + kdeg_sR_adc*C_sR_EXG*DAR - CL_PL_plasma * plasma_payload

    # payload concentration in tissue interstitium
    @views @. du.ints_payload = k_out*(end_cyto_payload-ints_payload)*V_cytosol[1:N_Organs]/V_IntS[1:N_Organs] - k_PL_ex*ints_payload
