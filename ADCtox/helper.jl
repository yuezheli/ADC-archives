...
function jones_init(TotalCells, RposFrac, Dose_in_ugkg, Rcopy, init_sR, k_endo, k_rec)
...
function TissueMass(sol, V_V, V_VM, V_E7, V_E6a, V_E7b, V_ISM, V_IntS)
    ...
    try
        ...
        [ (sol.u[i].tumor.AR_s).*(sol.u[i].tumor.Nc_1) for i in 1:length(sol.t)] ;
    end
...
function TissueConc(p_base, sol)
