...
sol = solve(prob, Rodas5(), reltol=1e-6, abstol=1e-9, saveat = 1.);

# visualization 
plt_plasma = plot(xlabel="Time (hr)", ylabel="Conc (nM)", yaxis = :log10, yticks = [1E-1, 1, 10, 1E2, 1E3], ylims = [1E-1, 1E3]);
plot!(sol.t, (sol[pbpk.C_Plasma_Albud] + sol[pbpk.C_Plasma_Exo]) * 1E9, label = "sims");
plot!(plasma_data.time_d * hr_per_day, plasma_data.conc_nM, seriestype = :scatter, label = "Overgaard et al., 2019"); 
display(plt_plasma)

savefig(plt_plasma, @projectroot("deliv/figure/plasma-pk-semaglutide-homo.png"));
