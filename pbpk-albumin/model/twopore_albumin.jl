# date: 2/22/2026 
# author: Yuezhe Li 
# purpose of this code: to code up mouse albumin PK model (in preparation of incorporating albumin:PDC binding in the future)
# default parameters were for mouse 
# https://pubmed.ncbi.nlm.nih.gov/38691205/

function albumin_pbpk_mtk(; name)
    ...
    @variables C_Plasma(t)=C_0_endo_Alb C_LN(t)=Epsilon        # Endogenous
    @variables C_Plasma_Exo(t)=0.0 C_LN_Exo(t)=0.0           # Exogenous
    ...
    vars = [C_Plasma; C_Plasma_Exo; C_LN; C_LN_Exo; CV_vars; CV_Exo_vars; CIS_vars; CIS_Exo_vars; UB_vars; UB_Exo_vars; B_vars; B_Exo_vars; F_vars]

    # Derived Molecular Constants
    ...
    for (i, org) in enumerate(orgs)
        ...
        # Two-pore permeability products (PS) and clearances (CLtp) calculation
        # Simplified for MTK logic structure
        ...
        if org == :Lung
            push!(eqs, D(CV) ~ ((Q_org + J_org)*C_Plasma - Q_org*CV - (CLtp + Spino_alb*CLup)*CV + 2*CLup*FR*CEB)/VV )
            push!(eqs, D(CV_X) ~ ((Q_org + J_org)*C_Plasma_Exo - Q_org*CV_X - (CLtp + Spino_alb*CLup)*CV_X + 2*CLup*FR*CEB_X)/VV )
        elseif org == :Kidney
            push!(eqs, D(CV) ~ (Q_org*CV_vars[Lung] - (Q_org - J_org)*CV - (CLtp + Spino_alb*CLup)*CV + 2*CLup*FR*CEB - GFR*theta*(1-Frea_alb)*CV)/VV)
            push!(eqs, D(CV_X) ~ (Q_org*CV_Exo_vars[Lung] - (Q_org - J_org)*CV_X - (CLtp + Spino_alb*CLup)*CV_X + 2*CLup*FR*CEB_X - GFR*theta*(1-Frea_alb)*CV_X)/VV)
        elseif org == :Liver
            ...
            push!(eqs, D(CV) ~ ( influx_endo - efflux_endo - (CLtp + Spino_alb*CLup)*CV + 2*CLup*FR*CEB)/VV )
            push!(eqs, D(CV_X) ~ ( influx_exo - efflux_exo - (CLtp + Spino_alb*CLup)*CV_X + 2*CLup*FR*CEB_X)/VV )
        else
            # normal organs
            push!(eqs, D(CV) ~ (Q_org*CV_vars[Lung] - (Q_org - J_org)*CV - (CLtp + Spino_alb*CLup)*CV + 2*CLup*FR*CEB)/VV )
            push!(eqs, D(CV_X) ~ (Q_org*CV_Exo_vars[Lung] - (Q_org - J_org)*CV_X - (CLtp + Spino_alb*CLup)*CV_X + 2*CLup*FR*CEB_X)/VV )
        end
        
        push!(eqs, D(CIS) ~ (CLtp*CV - (1-sigma_IS)*J_org*CIS - Spino_alb*CLup*CIS + 2*CLup*(1-FR)*CEB)/VIS )
        push!(eqs, D(CIS_X) ~ (CLtp*CV_X - (1-sigma_IS)*J_org*CIS_X - Spino_alb*CLup*CIS_X + 2*CLup*(1-FR)*CEB_X)/VIS )
    ...
    push!(eqs, D(C_LN) ~ (l_drain - L_Lymph_drain*C_LN)/V_LN)
    CIS_map_exo = Dict(zip(orgs, CIS_Exo_vars))
    l_drain_exo = sum([(1-sigma_IS)*(Q_map[o]*0.002)*CIS_map_exo[o] for o in orgs])
    push!(eqs, D(C_LN_Exo) ~ (l_drain_exo - L_Lymph_drain*C_LN_Exo)/V_LN)
