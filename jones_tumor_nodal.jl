...
# tumor model was largely adopted from Cami project, with tumor growth model implemented as exponential growth 
...
function jones_tumor_nodal!(du,u,p,t)
    ...
    @unpack Nc_1, Nc_2, Nc_3, Nc_4, Nc_1_neg, Nc_2_neg, Nc_3_neg, Nc_4_neg, R_s, R_e, AR_s, AR_e, P_c, P_m, P_neg_c = tumor

    @unpack tdouble, tau, PS_Score, PS_kd, KD6_WT, DAR, tumorPLQ, thalf_sR, thalf_sR_adc, init_sR, tumor_cell_radius, 
        Kkill_scale, Emax_Payload, IC50_Payload, k_PL, Rcopies, k_deg, k_rec, k_endo, Kon, Kd, k_out, k_in, k_lys, infusion = p

    # calculate tumor volume 
    Vc = 4/3 * pi * (tumor_cell_radius*1E-6)^3 * 1E3 ; # [L]
    TumorVolume = (Nc_1 + Nc_2 + Nc_3 + Nc_4 + Nc_1_neg + Nc_2_neg + Nc_3_neg + Nc_4_neg) * Vc;
    ...
    # du.C_EXG_Plasma = ((w_other_1_EXG_Plasma)/V_Plasma);
    du.C_EDG_Plasma = 0.0;

    # ; flux related to soluble receptor dynamics in plasma 
    ...
    du.C_EXG_Plasma = ((w_other_1_EXG_Plasma)/V_Plasma - Kon*C_sR_plasma*C_EXG_Plasma + Koff_sR_EXG*C_sR_EXG) + infusion;  # factor in soluble receptor degredation
    ...
    flux_A_R_s_binding = C_EXG_LN*R_s*Kon
    ...
    du.C_EXG_LN = ( du.C_EXG_LN + Du_A_m/V_LN ) 
