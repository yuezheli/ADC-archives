...
function jones_tumor_as_another_organ!(du,u,p,t)
    ...
    # add parameters for tumor data
    Vc = 1760.0*((1e-4)^3)/1000 # volume per cell [um^3 -> cm^3(mL) -> L]

    # # define avogadro constant if it has not been defined
    try 
        eval(N_av);
    catch
        N_av = 6.0221409e+23;
    ...
    A_m = C_EXG[Tumor,IntS] * V_IntS[Tumor] # unit: umol
